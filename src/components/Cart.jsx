...
console.log(cartItems)
  ...
  return (
    <>
      ...
      >
        <div className="flex justify-between items-center ">
          ...
          <IoMdClose
            ...
            className="border-2 border-grey-600 text-gray-600 font-bold p-1 text-xl rounded-md hover:text-red-300 hover:border-red-300 cursor-pointer"
          />
        </div>
        ...
        //   return <ItemCard key={food.id} id={food.id} name={food.name} price={food.price}  img={food.img} qty={food.qty}/>;
          ...
          return <ItemCard key={food.id} id={food.id}  img={food.img} name={food.name} price={food.price} qty={food.qty} />;
        ...
        <div className="absolute bottom-0">
          ...
          <h3 className="font-semibold text-grey-800">Total Amount: {totalPrice}</h3>
      ...
      <FaShoppingCart
        onClick={() => setActiveCart(!activeCart)}
        className={`rounded-full bg-white shadow-md text-5xl p-3 fixed bottom-10 right-4 ${totalQty > 0 && "animate-bounce delay-500 transition-all"}`}
      />
    </>
  );
}
